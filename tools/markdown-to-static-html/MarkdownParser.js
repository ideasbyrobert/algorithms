class MarkdownParser {
  ...
  isTable(lines, index) {
    return lines[index].trim().startsWith('|') && index + 1 < lines.length && /^\|(?:\s*:?-{3,}:?\s*\|)+\s*$/.test(lines[index + 1].trim());
  }
  ...
  isSpecialBlock(lines, index) {
    ...
    return this.isBlank(line) || this.isHeading1(line) || this.isHeading2(line) || this.isRule(line) || this.isEquation(line) || this.isTable(lines, index) || this.isUnorderedItem(line) || this.isOrderedItem(line);
  }
