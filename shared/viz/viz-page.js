 ...
 *        legend:     [{ className: 'solid', label: 'top row' }, { dot: true, label: 'current' }],
...
const VizPage = (() => {
     ...
     * @param {Array}  config.boundaries — boundary definitions (for frames, labels, values bar)
     ...
     * @returns {Object} DOM references: { grid, container, note, frames, labels, values, status }
     */
    ...
    function legendBlock(items) {
        ...
        const spans = items.map(i =>
            i.dot
                ...
                : `<span class="legend-item"><span class="legend-line ${i.className}"></span> ${i.label}</span>`
        );
    ...
    function vizContainer(boundaries, gridClass) {
        const lbls = boundaries.map(b =>
            `<span class="boundary-label label-${b.id}" id="label${cap(b.id)}">${b.label} <span class="bval">0</span></span>`
        ).join("\n");
    ...
    function timeline() {
        return [
            ...
            '</div>',
            '<input id="stepSlider" type="range" min="-1" max="0" value="-1" aria-label="Step scrubber">',
            '</div>'
        ].join("\n");
    }
    ...
    function infoGrid(statusItems, valueItems) {
        const sCells = statusItems.map(s =>
            `<div class="status-item"><small>${s.label}</small><span id="${s.id}Display" class="status-value">\u2014</span></div>`
        ).join("\n");
        const vCells = valueItems.map(v =>
            `<div class="val-cell ${v.accent || ""}"><small>${v.label}</small><strong id="${v.id}Val">0</strong></div>`
        ).join("\n");
        return [
            '<div class="info-grid">',
